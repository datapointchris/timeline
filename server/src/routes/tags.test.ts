...
describe('Tags API', () => {
  ...
  describe('POST /api/tags/events/:eventId/tags', () => {
    ...
    it('should add a tag to an event', async () => {
      const response = await request(app).post('/api/tags/events/test-event/tags').send({ name: 'philosophy' })
    ...
    it('should return 404 for non-existent event', async () => {
      const response = await request(app).post('/api/tags/events/non-existent/tags').send({ name: 'tag' })
  ...
  describe('DELETE /api/tags/events/:eventId/tags/:tagId', () => {
    it('should remove a tag from an event', async () => {
      ...
      const deleteResponse = await request(app).delete(`/api/tags/events/tagged-event/tags/${tagId}`)
